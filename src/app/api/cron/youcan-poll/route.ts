...
export async function GET(request: Request) {
  ...
  try {
    const result = await withCronMonitoring("youcan-poll", async () => {
      const results: Array<{
        merchantId: number;
      ...
      return { merchantsPolled: results.length, totalRecovered: totalNew, results };
    });

    return NextResponse.json({ data: result });
  } catch (err) {
    console.error("[YouCan Poll] Error:", err);
    return NextResponse.json(
      { error: "YouCan poll cron failed" },
      { status: 500 }
    );
  }
}
