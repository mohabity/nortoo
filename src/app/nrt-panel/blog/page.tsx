...
export default function AdminBlogPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [statsRes, articlesRes] = await Promise.all([
        fetch("/api/nrt-panel/blog/stats"),
        fetch("/api/nrt-panel/blog/articles?page=1"),
      ]);
  ...
  async function handleAction(action: string, extra?: Record<string, unknown>) {
    ...
    try {
      const res = await fetch("/api/nrt-panel/blog/actions", {
        method: "POST",
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {articles.map((a) => (
              <tr
                key={a.id}
                onClick={() => router.push(`/nrt-panel/blog/${a.id}`)}
