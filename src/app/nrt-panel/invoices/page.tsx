...
export default function AdminInvoicesPage() {
  ...
  const fetchInvoices = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/nrt-panel/invoices?${params}`);
  ...
  const handleMarkPaid = async () => {
    ...
    try {
      const res = await fetch(`/api/nrt-panel/invoices/${payModal.id}`, {
        method: "PUT",
  ...
  const handleCancel = async (inv: Invoice) => {
    ...
    try {
      const res = await fetch(`/api/nrt-panel/invoices/${inv.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
