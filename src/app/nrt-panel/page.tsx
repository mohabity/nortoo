...
export default function AdminOverviewPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [overviewRes, upgradesRes] = await Promise.all([
        fetch("/api/nrt-panel/overview"),
        fetch("/api/nrt-panel/invoices?type=upgrade&status=pending"),
      ]);
      if (!overviewRes.ok) {
  ...
  async function extendTrial(merchantId: number) {
    ...
    try {
      await fetch(`/api/nrt-panel/merchants/${merchantId}/actions`, {
        method: "POST",
      ...
      {pendingUpgrades.count > 0 && (
        <div className="rounded-sm bg-white border border-purple-200 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => router.push("/nrt-panel/invoices")}
